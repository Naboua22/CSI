<x-app-layout>
    ...
    <div>
        <div class="max-w-7xl sm:px-6 lg:px-8 lg:mx-auto mx-4">
            ...
            <form class="space-y-8 divide-y divide-gray-200" method="POST" action="{{ route('ECU.store') }}" >
                @csrf
                ...
                <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5">
                    <div>
                        ...
                        <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                            <label for="ue_id" class="block text-md text-gray-700 sm:mt-px sm:pt-2">
                                UE
                            </label>
                            
                            <select name="ue_id" id="ue_id" class=" appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                                @foreach ($ues as $ue)<option value="{{$ue->id}}">{{$ue->nom}}</option>@endforeach
                            </select>
                        </div>
                        ...
                        <div class="space-y-6 sm:space-y-5">
                            ...
                            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                <label for="nom_enseignant" class="block text-md text-gray-700 sm:mt-px sm:pt-2">
                                    Nom Enseignant
                                </label>
                                <div class="mt-1 sm:mt-0 sm:col-span-2">
                                <input type="text" name="nom_enseignant" id="nom_enseignant" autocomplete="given-name" class="block w-full  border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                </div>
                            </div>
